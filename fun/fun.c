...
struct fun *
lookup_fun (char *name)
{

  struct fun *traverse;

  for (traverse = fun_hash_table[make_hash ((unsigned char *) name)];
       traverse; traverse = traverse->next)

    if (!strcmp (traverse->name, name))
      return traverse;
...
struct fun *
install_fun (char *name, unsigned long start, unsigned long end, int num)
{
...
display_fun (struct fun *ptr)
{
  ...
  err (0,
       "Function name:%s\nNumber of Arguments:%d\nFunction start address:%#x\nFunction End Address:%#x:",
       ptr->name, ptr->args, ptr->start, ptr->end);
...
struct data *
function_handler (struct fun *fun_ptr, unsigned long lookup_stop, int scope)
{
...
push_addr (unsigned long addr)
{
  ...
  var->value->ele_type = _DBL;

  var->value->DBL_PTR = INSTALL_DOUBLE ((double) addr);
...
struct data *
function_prologue (struct fun *fun_ptr)
{
...
function_epilogue (void)
{
  ...
  stack_frame = (struct stack *) (unsigned long) (var->value->DBL_VALUE);
  ...
  reference = (char *) (unsigned long) (var->value->DBL_VALUE);	//set up the reference
