...
int
do_print (char *buf, const char *fmt, struct argument *ptr)
{
  int len;
  unsigned long num;
  int i, base;
  ...
  for (str = buf; *fmt; ++fmt)
	...
	case 'c':
	  if (!(flags & LEFT))
	    while (--field_width > 0)
	      *str++ = ' ';
	  *str++ =
	    (unsigned char) EVAL_DATA (arg_next (ptr, _INT), INT_VALUE);
	  while (--field_width > 0)
	    *str++ = ' ';
	  continue;
	...
	case 'f':
	  ...
	  len =
	    sprintf (buffer, "%*.*f", field_width, precision,
		     EVAL_DATA (arg_next (ptr, _FLT), FLT_VALUE));
	  if (!(flags & LEFT))
	    while (len < field_width--)
	      *str++ = ' ';
...
struct data *
my_generic_printf (struct argument *ptr, int flag)
{
  struct data *dp;
  int i;
  char buffer[BUFFER + 1];
