...
struct keywords *lookup_keyword( char *keyword) {

  struct keywords *ptr;
  for(ptr = &keywords[0];ptr->keyword;ptr ++) 
    if(!strcmp(ptr->keyword,keyword) ) return ptr;

  return (struct keywords *)NULL; 

}

struct intern *lookup_intern(char *keyword) {
...
void my_strncpy(char *dst,char *src,unsigned int bytes) {
...
int my_atoi(char *s) {
  ...
  for(result=0;isxdigit(*s); s++) {
    ...
    value = isdigit(*s) ? *s - '0' : (  islower(*s) ? toupper(*s) : *s) - 'A' +  10  ;
    if(value >= base) {
    ...
    switch(get_token()) { 
	 ...
	 /*Now Check for Functional Syntaxes*/
	   ...
	   default:
             ...
             err(0,"Function Precursor:Syntax Error at line %d",lines+1);
           ...
           case IDENTIFIER:
               ...
               switch(get_token() ) { //get a token again
                   ...
                   case LP: //hit a Left Parenthesis
                        ...
                        if(current_token != LB) 
                          ...
                          while(nb) { //parsing a block

                            t= get_token();

                            if (t==LB) ++nb;

                            else if(t==RB)  --nb; //decrement the number braces
		...
		SET_ERROR(err_type);
           ...
           if(! install_fun(fun_name,start,end,num) ) {
              ...
              err(0,"Function Precuror: Error in installing Function:\n");
             ...
             #ifdef DEBUG
                err(0,"Finished Installing Function %s at Start Address:%#x and End Address:%#x",fun_name,start,end);
...
struct data *return_handler(unsigned long lookup_stop,int scope,int state,int send_status,int *loop_flag) {
...
struct data *fun_handler(unsigned long lookup_stop,int scope,int state,int send_status,int *loop_flag){
...
int do_cond(unsigned long lookup_stop,int scope,int send_status) {
  extern char *reference;
  char *old;
  struct data *ptr;
  ...
  if(! send_status) return send_status;
...
void skip_block() {
  ...
  while(lb) {
    get_token();
    if(current_token == LB) lb ++;
    else if(current_token == RB) lb--;
  }
...
void skip_statement(){
  ...
  while(get_token() ) {
    ...
    if(current_token == TERMINATOR) break;
...
struct data *if_handler(unsigned long lookup_stop,int scope,int state,int status,int *loop_flag) {
  ...
  if(! status) 
    ...
    if (send_status) {
     ...
     if(ptr || *loop_flag == BREAK || *loop_flag == CONTINUE )  return ptr;
	  ...
	  send_status = do_cond(lookup_stop,scope,status);
	    ...
	    if(! last_status && send_status) {
	      ...
	      last_status = send_status; //reset last status 
              ...
              if(ptr || *loop_flag == BREAK || *loop_flag == CONTINUE) break;
	    ...
	    if(! last_status) {
               ...
               ptr=block_parser(lookup_stop,scope,state,1,loop_flag);
...
struct data *break_handler(unsigned long lookup_stop,int scope,int state,int status,int *loop_flag) {
  ...
  if(status) *loop_flag = BREAK; //set the break flag for active states
...
struct data *continue_handler(unsigned long lookup_stop,int scope,int state,int status ,int *loop_flag){
  ...
  if(status) *loop_flag = CONTINUE;
  return ptr;
}   
 
/* The while loop handler */

struct data *while_handler(unsigned long lookup_stop,int scope,int state,int status,int *lf) {
  int send_status;
  extern char *reference;
  char *old,*jmp_point;
  int loop_flag =ON;
  ...
  if(! status ) return block_parser(lookup_stop,scope,state,status,&loop_flag); //syntax checker
  ...
  while(send_status) { 
    ...
    if( loop_flag == BREAK) break;

    if(loop_flag == CONTINUE) loop_flag = ON;
...
void skip_while(char *old,unsigned long lookup_stop,int scope,int send_status) {
  extern char *reference;
...
struct data *dowhile_handler(unsigned long lookup_stop,int scope,int state,int send_status,int *lf) {
  ...
  if(jmp_point) reference = jmp_point;
...
void init_incr(unsigned long lookup_stop,int scope,int send_status,int init) {
  ...
  while(lp) {
    ...
    if(current_token == RP) --lp;

    else if(current_token == LP) ++lp;
...
struct data *for_handler(unsigned long lookup_stop,int scope,int state,int send_status,int *lf) {
  ...
  if(current_token != LP) {
   ...
   return block_parser(lookup_stop,scope,state,send_status,&loop_flag) ;
  ...
  while(exp_status) { 
    ...
    else if(! jmp_point) jmp_point = reference;  //save the jmp point
...
struct argument *builtin_handler(unsigned long lookup_stop,int scope) {
...
struct argument *arg_ptr = arg_node();
  ...
  while(reference < (char*)stop_addr) {  
    ...
    arg_install(arg_ptr,(char*)NULL,exp_evaluate(stop_addr,lookup_stop,scope) );
    args++;
  }
...
struct data *output_handler(unsigned long lookup_stop,int scope,int status,int flag) {
...
struct data *printf_handler(unsigned long lookup_stop,int scope,int status){
...
struct data *sprintf_handler(unsigned long lookup_stop,int scope,int status){
...
void get_stop_addr(unsigned long *stop_addr) {
  ...
  while(lp) { 
    ...
    if(current_token == LP) ++lp;

    else if(current_token == RP) --lp;

    else if(current_token == TERMINATOR || current_token == NEWLINE || current_token == END) {
      SET_ERROR(SYNTAX_ERROR);
    }
...
struct data *block_parser(unsigned long lookup_stop,int scope,int state,int send_status,int *loop_flag) { 
  /* Parse a block */
  ...
  while(nb) { //go inside a block
    ...
    if(current_token == NEWLINE || current_token == TERMINATOR ) continue;
	...
	dp = builtin->intern_ptr(lookup_stop,scope,send_status);
      ...
      if(k) {
        ...
        dp = k->keyword_ptr(lookup_stop,scope,state,send_status,loop_flag);
	  ...
	  if(state) 
        ...
        else if(*loop_flag == BREAK || *loop_flag == CONTINUE) return dp;
    ...
    if(current_token != EQUAL) { //then simply call the expression parser
	...
	show_data(dp);
      ...
      else skip_statement() ; //syntax checker
       
    }
      
    else {  //the send_status patch
  ...
  if(state && *loop_flag == OFF) return zero_object();
